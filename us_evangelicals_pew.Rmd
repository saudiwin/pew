...
exp_cum_sum <- rowSums(exp(t(t(as.matrix(reltrad_slope)) %*% t(as.matrix(cbind(abil1,abil2)))) %*% t(as.matrix(reltrad_coefs)) + matrix(rep(reltrad_diff,1000000),nrow=1000000,ncol=length(reltrad_diff),byrow=TRUE)))
indiv_sum <- lapply(1:length(reltrad_coefs), function(x) {
  y <- exp(reltrad_coefs[x] * t(t(as.matrix(reltrad_slope)) %*% t(as.matrix(cbind(abil1,abil2)))) + reltrad_diff[x])
})
...
max_probs <- sapply(indiv_probs,function(x) 
  which(x==max(x)))
max_vals <- cbind(abil1,abil2)[max_probs,]
reltrad_labels <- levels(data$RELTRAD)[2:length(levels(data$RELTRAD))]
# reltrad_data <- data_frame(dim1=reltrad_slope[[1]]*reltrad_coefs[reltrad_coefs!=0],
#                            dim2=reltrad_slope[[2]]*reltrad_coefs[reltrad_coefs!=0],
#                         labels=reltrad_labels)
reltrad_data <- data_frame(dim1=max_vals[-1,1],
                           dim2=max_vals[-1,2],
                           labels=reltrad_labels)
reltrad_data %>% plot_ly(type='bar',x=~dim1,y=~reorder(labels,dim1),orientation='h',text=~labels) %>% layout(xaxis=list(title='Religous Belief (Liberal to Conservative)'), yaxis=list(title=''),margin=list(l=100),title='Relative Position of Belief in Religious Communities')
```
...
reltrad_diff <- nominal_coefs$RELTRAD[grepl(pattern='d[0-9]+',x=attr(nominal_coefs$RELTRAD,'dimnames')[[2]])]
# there is no way to calculate this equiprobability directly, so we simulate it using random abilities
abil1 <- sample(seq(rectangle_scores['min_x'],rectangle_scores['max_x'],length.out=10000000),1000001)
abil2 <- sample(seq(rectangle_scores['min_y'],rectangle_scores['max_y'],length.out=10000000),1000001)
exp_cum_sum <- rowSums(exp(t(t(as.matrix(reltrad_slope)) %*% t(as.matrix(cbind(abil1,abil2)))) %*% t(as.matrix(reltrad_coefs)) + matrix(rep(reltrad_diff,1000001),nrow=1000001,ncol=length(reltrad_diff),byrow=TRUE)))
indiv_sum <- lapply(1:length(reltrad_coefs), function(x) {
  y <- exp(reltrad_coefs[x] * t(t(as.matrix(reltrad_slope)) %*% t(as.matrix(cbind(abil1,abil2)))) + reltrad_diff[x])
})
indiv_probs <- lapply(indiv_sum,function(x) {
...
```{r,echo=FALSE,  message=FALSE, warning=FALSE,fig.width=10,fig.height=6}
 sample_scores %>% plot_ly(y=~F2,x=~F1,text=~data_labels,colors='Greens') %>% add_markers(hoverinfo='text',name='All Respondents',opacity=0.2,size=1) %>% 
  layout(annotations=a,xaxis=list(title="",showticklabels = FALSE),
         yaxis=list(title="",showticklabels = FALSE),
         title='Both Education and Income') %>% add_lines(data=all_points_income,y=~ypoints,x=~xpoints,color=~categories,linetype=~categories,name='Income Categories',text=~categories,colors=I("Reds")) %>% add_lines(data=all_points_educ,y=~ypoints,x=~xpoints,color=~categories,inherit=FALSE,name='Education',
                                                                             text=~categories) 
```
...
to_choose <- floor(nrow(all_points)/length(agerec_labels))
...
sample_scores %>% plot_ly(y=~F2,x=~F1,text=~data_labels,colors='Greens') %>% add_markers(hoverinfo='text',name='All Respondents',opacity=0.2,size=1) %>% 
  layout(annotations=a,xaxis=list(title="",showticklabels = FALSE),
         yaxis=list(title="",showticklabels = FALSE),
         title='Age & Evangelicals',showlegend=FALSE) %>% add_lines(data=all_points,y=~ypoints,x=~xpoints,color=~categories,inherit=FALSE,name='agerecation',
                                                           text=~categories) %>% 
  add_text(data=text_data,x=~dim1,y=~dim2,text=~labels,name='Point Labels',textposition='bottom',color=I('gray30'),showlegend=FALSE) %>% add_lines(data=all_points_evan,y=~ypoints,x=~xpoints,inherit=FALSE,
                                                           text=~categories,color=I('gray30'),linetype=I(2)) %>% 
