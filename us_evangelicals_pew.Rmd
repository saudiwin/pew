...
Next we'll put education on the same plot so that we can look at income and education together. This plot validates some of what we know about education--more educated people tend to be more liberal--while in also says something new. In this plot, less-educated people tend to be less conservative in terms of belief than more-educated people. 
...
educ_data <- data_frame(dim1=dims[1,],
                           dim2=dims[2,],
                        labels=educ_labels)
 ...
 person_scores %>% sample_n(1500) %>% plot_ly(y=~F2,x=~F1,text=~data_labels) %>% add_markers(hoverinfo='text',name='All Respondents',opacity=0.1) %>% 
  layout(annotations=a,xaxis=list(title='Religious Belief',showticklabels = FALSE),
         yaxis=list(title='Political Opinion',showticklabels = FALSE)) %>% add_markers(data=educ_data,y=~dim2,x=~dim1,inherit=FALSE,name='Education') %>% add_text(data=educ_data,x=~dim1,y=~dim2,text=~labels,name='Point Labels',textposition='bottom right')
```
