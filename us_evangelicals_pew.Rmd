...
levels(data$educ) <- c("Less than high school (Grades 1-8 or no formal schooling)",
...
data$educ <- factor(data$educ,labels=c('Middle School','Some HS','HS Grad','Some College','2-yr Degree','4-yr Degree','Some Grad School','Grad Degree'))
...
text_data <- data_frame(dim1=dims[1,],dim2=dims[2,],labels=income_labels)
 ...
 person_scores %>% sample_n(1500) %>% plot_ly(y=~F2,x=~F1,text=~data_labels) %>% add_markers(hoverinfo='text',name='All Respondents',opacity=0.1) %>% 
  layout(annotations=a,xaxis=list(title='Religious Belief',showticklabels = FALSE),
         yaxis=list(title='Political Opinion',showticklabels = FALSE)) %>% add_lines(data=income_data,y=~ypoints,x=~xpoints,color=~categories,inherit=FALSE,name='Income Categories') %>% add_text(data=text_data,x=~dim1,y=~dim2,text=~labels,name='Point Labels',textposition='bottom left')
```

Next we'll put education on the same plot so that we can look at income and education together. This plot validates some of what we know about education--more educated people tend to be more liberal--while in also says something new. In this plot, less-educated people tend to be less conservative in terms of belief than more-educated people. Of course, this should be interpreted with some caution because our definition of ``conservative belief" is somewhat broad. However, there does appear to be an interesting relationship between income and education. People who have more education are more liberal politically, while people with more income are more conservative politically. 
...
We can look at both simultaneously by putting the lines on the same chart so that we can divide up ou respondents into little boxes. (Yes, we are literally putting people in boxes). Nowe we have a bunch of pick-up sticks laid on top of our chart. While it is a bit too much to absorb, it shows how income and education have a complicated interaction when it comes to people's religious and political beliefs. To give just one interesting interpretation, there is a big box of people who have a very high income (more than $150K) but low education (high school or less). These people tend to have very conservative politics but their religious beliefs are moderate or even liberal. On the flip side, the box with people with high income and high education shows that they are more liberal politically, although they also have more conservative beliefs. In other words, people are complicated.
...
Now we're going to dig deep into how people identify in terms of religion and what that says about their politics. 